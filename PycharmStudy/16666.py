...
def my_sort(lst, rev=False):
    ...
    if rev:
        ...
        return my_sort(left, True) + [token] + my_sort(right, True)
        ...
        return my_sort(left) + [token] + my_sort(right)
...
for test_case_num in range(1, t + 1):
    ...
    sorted_lst = my_sort(num_lst)
    rev_sorted_lst = my_sort(num_lst, True)
